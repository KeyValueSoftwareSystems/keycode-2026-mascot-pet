...
module.exports = {
  ...
  mac: {
     ...
     * Ad-hoc signing.
     *
     * There is no Developer ID (internal-only distribution, locked decision 4). Without *any*
     * signature, Gatekeeper on Apple Silicon refuses the app with "damaged and can't be opened",
     * which reads as a broken build rather than an unsigned one. `identity: '-'` produces an ad-hoc
     * signature that avoids that specific dialog.
     */
    identity: '-',
